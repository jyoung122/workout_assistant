...
export default function WorkoutProgress({ selectedDate }) {
  ...
  useEffect(() => {
    const fetchTodayWorkouts = async () => {
      ...
      try {
        const res = await fetch(`workoutassistant-production.up.railway.app/workouts/${formattedDate}`, {
          method: "GET",
          headers: {
        ...
        const responses = await Promise.all(
          todayExercises.map(async (exercise) => {
            const res = await fetch(
              `workoutassistant-production.up.railway.app/workouts/history/${timeframe}?exercise=${encodeURIComponent(exercise)}`,
              {
                method: "GET",
                headers: {
                  "Authorization": `Bearer ${token}`,
